...
import de.prkmd.espgame.level.Planet;
...
public class Kanone extends Entity {
	...
	public void updatePosition() {
		...
		top.setRotation(alpha + beta + 90f);
		farbBackground.setCenter(position.x - MathUtils.cosDeg(beta + alpha) * 16,
				position.y - MathUtils.sinDeg(beta + alpha) * 16);
		...
		if (shoot) {
			...
			if (cooldown == 0 && kraft > MINFORCE) {
				...
				if (hasAmmo(selectedEddy)) {
					...
					pop.stop();
					pop.play(ESPGame.game.getSoundVolume()*4f);
					...
					ESPGame.getLevel().createTextDisplayer(getPosition(), VectorUtils.randomNormalized().scl(0.5f),
							NO_AMMO_DUR, NO_AMMO_TEXT, Fontsize.Klein);
				}

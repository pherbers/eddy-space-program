...
public abstract class Entity {
    ...
    public boolean checkCollision(Entity e) {
        if (collidable
                && e.collidable
                && position.dst2(e.getPosition()) < MathUtils.sqr(radius + e.getRadius()))
            return true;
        return false;
    }

    public void pushAway(Vector2 origin, float force) {
        Vector2 v = new Vector2(origin.x - position.x, origin.y
                - position.y);
        v.scl(force / v.len());
        velocity.add(v);
    }
    ...
    /*public boolean isInScreen() {
        int y = (int) (Math.abs(Game.getRenderHeight()) / Game.getGamescale());
        int x = (int) (Math.abs(Game.getRenderWidth()) / Game.getGamescale());

...
public class Kanone extends Entity {
	...
	public Kanone(float omega) {
		...
		position = new Vector2(-cosalpha * (planet.getRadius() + 16), -sinalpha * (planet.getRadius() + 16));
		explodeable = false;
		collidable = false;
		beta = 0;
		kraft = 0;
		...
		// farbe = farbe.transform(Transform.createRotateTransform((float)
		// alpha));
		// setzeFarbe();
	...
	/*
	 * private void setzeFarbe() { int x = ESPGame.getLevel().getSelectedEddy();
	 * switch (x) { case 0: farbeFill = org.newdawn.slick.Color.red; break; case
	 * 1: farbeFill = org.newdawn.slick.Color.blue; break; case 2: farbeFill =
	 * org.newdawn.slick.Color.green; default: break; }
	 * 
	 * }
	 */
	...
	private void setDrehung(float winkel) {
		...
		/*
		 * farbe = new Rectangle(15, -15, 30, 30); farbe =
		 * farbe.transform(Transform.createTranslateTransform( (float)
		 * (planet.getRadius()), 0)); farbe = farbe.transform(Transform
		 * .createRotateTransform((float) alpha)); farbe =
		 * farbe.transform(Transform.createRotateTransform( (float) beta,
	...
	public void render(SpriteBatch batch) {
		...
		/*
		 * if (Game.DEBUG) { g.drawRect((float) position.getX(), (float)
		 * position.getY(), 10, 5); g.drawLine((float) position.getX(), (float)
		 * position.getY(), (float) (position.getX() + Math.cos(alpha + beta) *
		 * kraft 10), (float) (position.getY() + Math.sin(alpha + beta) * kraft
		 * 10));
		 * 
	...
	public void update() {
		...
		if (shoot) {
			...
			if (cooldown == 0) {
				...
				/*
				 * int selectedEddy = Game.getLevel().getSelectedEddy(); switch
				 * (selectedEddy) { case 0: farbe = Color.ROT; break; case 1:
				 * farbe = Color.BLAU; break; case 2: farbe = Color.GRUEN;
				 * break; default: farbe = Color.ROT; break; }
				 */
				// if (hasAmmo(selectedEddy)) {
				Eddy eddy = new Eddy(position.x + x / (float) Math.hypot(x, y) * 64,
						position.y + y / (float) Math.hypot(x, y) * 64, x, y, farbe,
						ESPGame.getLevel().getPlanet().getOrbit()); // Farbe
				// bestimmen
				ESPGame.getLevel().addEddy(eddy);
	...
	/*
	 * public boolean hasAmmo(int farbe) { int c; switch (farbe) { case 0: c =
	 * Level.level().getReserveRot(); break; case 1: c =
	 * Level.level().getReserveBlau(); break; case 2: c =
	 * Level.level().getReserveGruen(); break; default: throw new

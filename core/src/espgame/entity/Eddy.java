...
public class Eddy extends Entity {
    ...
    public Eddy(float x, float y, float vx, float vy, Color farbe,
                Orbit orbit) {
        super(x, y);
        setVelocity(vx, vy);
        this.orbit = orbit;
...
//                g.drawLine((float) getX(), (float) getY(), 0, 0);
    ...
    public static Eddy joinEddys(Eddy e1, Eddy e2) {
        ...
        Eddy joinedEddy = new Eddy(x, y, vx, vy, joinColor(e1.farbe, e2.farbe),
                e1.getOrbit());
        // TODO: Sounds
...
//            Sounds.POP.play();
    ...
    /*private void setEmitterColor(Color color) {
        ...
        ConfigurableEmitter.ColorRecord cr = (ConfigurableEmitter.ColorRecord) (emitter.colors
                .get(0));
        cr.col = toSlickColor(color);
        cr = (ConfigurableEmitter.ColorRecord) (emitter.colors.get(1));
        cr.col = org.newdawn.slick.Color.white;
    }*/
    ...
    /*private org.newdawn.slick.Color toSlickColor(Color c){
        ...
        switch (c) {
            case ROT:
                col = org.newdawn.slick.Color.red;
                break;
            case GRUEN:
                col = org.newdawn.slick.Color.green;
                break;
            case BLAU:
                col = org.newdawn.slick.Color.blue;
                break;
            case GELB:
                col = org.newdawn.slick.Color.yellow;
                break;
            case CYAN:
                col = org.newdawn.slick.Color.cyan;
                break;
            case MAGENTA:
                col = org.newdawn.slick.Color.magenta;
                break;
            case WEISS:
                col = org.newdawn.slick.Color.white;
                break;
            case SCHWARZ:
                col = org.newdawn.slick.Color.black;
                break;
            default:
                col = org.newdawn.slick.Color.white;
        }
    ...
    private void setColor() {
        /*switch (farbe) {
            case ROT:
                sprite = Sprites.EDDY_ROT.getScaledCopy((int) radius * 2,
                        (int) radius * 2);
                break;
            case GRUEN:
                sprite = Sprites.EDDY_GRUEN.getScaledCopy((int) radius * 2,
                        (int) radius * 2);
                break;
            case BLAU:
                sprite = Sprites.EDDY_BLAU.getScaledCopy((int) radius * 2,
                        (int) radius * 2);
                break;
            case GELB:
                sprite = Sprites.EDDY_GELB.getScaledCopy((int) radius * 2,
                        (int) radius * 2);
                break;
            case CYAN:
                sprite = Sprites.EDDY_CYAN.getScaledCopy((int) radius * 2,
                        (int) radius * 2);
                break;
            case MAGENTA:
                sprite = Sprites.EDDY_MAGENTA.getScaledCopy((int) radius * 2,
                        (int) radius * 2);
                break;
            case WEISS:
                sprite = Sprites.EDDY_WEISS.getScaledCopy((int) radius * 2,
                        (int) radius * 2);
                break;
            case SCHWARZ:
                sprite = Sprites.EDDY_WEISS.getScaledCopy((int) radius * 2,
                        (int) radius * 2);
                sprite.setImageColor(0.5f, 0.5f, 0.5f);
                break;
...
//        emitter.wrapUp();
    ...
    public static Color joinColor(Color color1, Color color2) {
        if (color1 == null || color2 == null || color1 == Color.SCHWARZ
                || color2 == Color.SCHWARZ)
            return null;
        switch (color1) {
            case ROT:
                switch (color2) {
